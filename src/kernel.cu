...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf, const glm::vec3 *pos, const glm::vec3 *vel) {
  ...
  glm::vec3 rule1Velocity(0.0f);

  if (neighbors > 0) {
      perceivedCenter /= neighbors;

      rule1Velocity =
          (perceivedCenter - pos[iSelf]) * rule1Scale;
  }


  glm::vec3 rule2Velocity = separation * rule2Scale;

  glm::vec3 rule3Velocity(0.0f);

  if (rule3Neighbors > 0) {
      perceivedVelocity /= rule3Neighbors;
      rule3Velocity = perceivedVelocity * rule3Scale;
  }
...
__global__ void kernIdentifyCellStartEnd(int N, int *particleGridIndices,
  ...
  //last particle is always N
  if (index == N - 1) {
        gridCellEndIndices[currentGridIndex] = N;
  }
}

__global__ void kernShuffleCoherent(
    int N,
    ...
    pos2[index] = pos[sourceIndex];
    vel2[index] = vel1[sourceIndex];
 }
...
__global__ void kernUpdateVelNeighborSearchScattered(
  ...
  int neighborX = thisPos.x < cellCenterX ? gridX - 1 : gridX + 1;
  int neighborY = thisPos.y < cellCenterY ? gridY - 1 : gridY + 1;
  int neighborZ = thisPos.z < cellCenterZ ? gridZ - 1 : gridZ + 1;
    ...
    for (int yi = 0; yi < 2; yi++) {
        for (int zi = 0; zi < 2; zi++) {
            ...
            for (int i = start; i < end; i++) {
                ...
                int otherBoidIndex = particleArrayIndices[i];
                ...
                float distance =
                  glm::distance(thisPos, pos[otherBoidIndex]
                );

                //Rule 1
                if (distance < rule1Distance) {
                  perceivedCenter += pos[otherBoidIndex];
                  rule1Neighbors++;
                }
                ...
                if (distance < rule2Distance) {
                    separation -=
                        (pos[otherBoidIndex] - thisPos);
                }
                
                //Rule 3
                if (distance < rule3Distance) {
                    perceivedVelocity += vel1[otherBoidIndex];
                    rule3Neighbors++;
                }
            }
        }
...
__global__ void kernUpdateVelNeighborSearchCoherent(
  ...
  if (rule1Neighbors > 0) {
      ...
      velocityChange +=
          (perceivedCenter - thisPos) * rule1Scale;
  }
  ...
  glm::vec3 newVelocity =
      vel1[index] + velocityChange;

  float speed = glm::length(newVelocity);

  if (speed > maxSpeed) {
      newVelocity =
          glm::normalize(newVelocity) * maxSpeed;
  }
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  kernShuffleCoherent<<<fullBlocksPerGrid, blockSize>>>(
      numObjects,
      dev_particleArrayIndices,
      dev_pos,
      dev_vel1,
